...
export default function Services() {
  ...
  return (
    <section id="services" className="py-32 bg-[#0B0D13] relative text-white overflow-hidden">
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="mb-20">
          ...
          <h2 className="text-4xl md:text-6xl font-black tracking-tight mt-6 text-white">
            Engineered Services. <br />Structured for <span className="text-[#00FFFF]">Scale</span>.
          </h2>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          
          {/* LEFT SIDE NAVIGATION (Now scrolls or flows beautifully with all 6 items) */}
            ...
            {finalServicesStructure.map((service) => {
              ...
              return (
                <button
                  ...
                  className={`w-full text-left p-5 rounded-xl border transition-all duration-300 relative cursor-pointer outline-none ${
                    isSelected 
                      ? 'border-[#00FFFF]/40 bg-[#01257D]/25 shadow-[0_0_25px_rgba(0,255,255,0.06)]' 
                ...
                >
                  {isSelected && (
                    <motion.div 
                      ...
                      className="absolute inset-0 rounded-xl border border-[#00FFFF]/30 pointer-events-none"
                      transition={{ type: "spring", stiffness: 300, damping: 30 }}
                    />
                  )}
                  <span className={`text-xs block font-mono mb-1.5 ${isSelected ? 'text-[#00FFFF] font-bold' : 'text-gray-500'}`}>
                    {service.category.split(' / ')[0]}
                  </span>
                  <span className={`text-sm md:text-base font-bold tracking-wide ${isSelected ? 'text-white' : 'text-gray-400'}`}>
          ...
          <div className="lg:col-span-8 p-8 md:p-12 rounded-2xl border border-white/5 bg-gradient-to-b from-white/[0.02] to-transparent backdrop-blur-sm min-h-[550px] shadow-2xl">
            <AnimatePresence mode="wait">
              {finalServicesStructure.map((service) => {
                ...
                return (
                  ...
                  >
                    <div>
                      <h3 className="text-2xl md:text-3xl font-black text-white tracking-wide mb-3">
                        {service.title}
                      </h3>
                    ...
                    <div className="border-t border-white/10 pt-8 space-y-6">
                      {service.subServices.map((sub, idx) => (
                        <div key={idx} className="group flex gap-4 items-start border-b border-white/5 pb-5 last:border-0 last:pb-0">
                          <div className="w-1.5 h-1.5 rounded-full bg-[#00FFFF] mt-2 shadow-[0_0_8px_#00FFFF] shrink-0" />
                          <div>
                            <h4 className="text-md font-bold text-white group-hover:text-[#00FFFF] transition-colors duration-200">
                              {sub.name}
                            </h4>
