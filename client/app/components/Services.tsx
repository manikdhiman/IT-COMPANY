...
export default function Services() {
  ...
  return (
    <section id="services" className="py-32 bg-[#0B0D13] relative text-white">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
              ...
              return (
                <button
                  ...
                  className={`w-full text-left p-6 rounded-xl border transition-all duration-300 relative cursor-pointer ${
                    isSelected 
                      ? 'border-[#00FFFF]/40 bg-[#01257D]/30 shadow-[0_0_20px_rgba(0,255,255,0.08)]' 
                      : 'border-white/5 bg-white/[0.01] hover:border-white/10 hover:bg-white/[0.02]'
                  }`}
                >
                  <span className={`text-xs block font-mono mb-2 ${isSelected ? 'text-[#00FFFF] font-bold' : 'text-gray-500'}`}>
          ...
          <div className="lg:col-span-8 p-8 md:p-12 rounded-2xl border border-white/5 bg-gradient-to-b from-white/[0.02] to-transparent backdrop-blur-sm min-h-[480px]">
                    ...
                    <div className="border-t border-white/10 pt-8 space-y-6">
                      {service.subServices.map((sub, idx) => (
                        <div key={idx} className="group flex gap-4 items-start border-b border-white/5 pb-5 last:border-0 last:pb-0">
                          <div className="w-1.5 h-1.5 rounded-full bg-[#00FFFF] mt-2 shadow-[0_0_8px_#00FFFF]" />
                          <div>
                            <h4 className="text-md font-bold text-white group-hover:text-[#00FFFF] transition-colors">
                              {sub.name}
                            </h4>
